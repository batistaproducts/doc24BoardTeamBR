...
export default function StatusReportModal({
  ...
  // Load app parameters
  const parameters = useMemo(() => getAppParameters(), []);

  // Status options for MultiSelectFilter (all statuses from system parameters + any active in tasks)
  const statusOptions = useMemo(() => {
    const base = (parameters?.statuses || []).map(s => ({
      id: s.id,
      label: s.label,
      color: s.color
    }));
    const existingIds = new Set(base.map(s => s.id.toLowerCase().trim()));

    tasksState.forEach(item => {
      const st = item.task.status;
      if (st && st.trim() && !existingIds.has(st.trim().toLowerCase())) {
        existingIds.add(st.trim().toLowerCase());
        base.push({ id: st.trim(), label: st.trim(), color: '#64748b' });
      }
    });

    return base;
  }, [parameters, tasksState]);
  ...
  const visibleTasks = useMemo(() => {
    return tasksState.filter(item => {
      ...
      // Status filter (Multi-selection matching Board de Atividades)
      if (filterStatuses.length > 0) {
        const taskStatus = (t.status || '').toLowerCase().trim();
        const matches = filterStatuses.some(selected => selected.toLowerCase().trim() === taskStatus);
  ...
  }, [tasksState, searchQuery, filterStatuses, filterPriority, filterOwner]);
              ...
              <div className="pt-2 border-t border-slate-100 space-y-2">
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-2 items-center">
                  <div className="sm:col-span-7 relative">
                    ...
                    <input
                      type="text"
                      placeholder="Buscar por nome, ticket..."
                      value={searchQuery}
                      onChange={(e) => setSearchQuery(e.target.value)}
                      className="w-full text-xs pl-8 pr-2.5 py-1.5 bg-slate-50 border border-slate-300 rounded-lg text-slate-700 focus:ring-2 focus:ring-[#343180] outline-hidden h-[34px]"
                    />
                  </div>
                ...
                <div className="flex items-center justify-between text-[11px] pt-1">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {visibleTasks.length !== tasksState.length && (
                      <span className="text-slate-400 font-normal">
                        ({visibleTasks.length} {visibleTasks.length === 1 ? 'visível' : 'visíveis'})
                      </span>
                    )}
                    {(filterStatuses.length > 0 || searchQuery) && (
                      <button
                        type="button"
                        onClick={() => {
                          setSearchQuery('');
                        ...
                        className="text-[10px] text-amber-600 hover:text-amber-800 font-bold hover:underline cursor-pointer ml-1"
                      >
                        (Limpar filtros)
                      </button>
                    )}
